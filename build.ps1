...
function New-AppPackage {
    ...
    if (-not (Test-Path $Entry)) { throw "No se encontró el entrypoint: $Entry" }
    ...
    if ($code -ne 0) {
        ...
        Write-Warning "python -m PyInstaller falló (código $code). Probando ejecutable: $pyinstallerExe"
        & $pyinstallerExe '--noconfirm' '--clean' '--name' $name '--windowed' '--onefile' '--distpath' 'dist' '--workpath' 'build' '--specpath' 'build' $Entry
        $code = $LASTEXITCODE
        if ($code -ne 0) { throw "PyInstaller falló con código $code" }
    }
    $distRoot = 'dist'
